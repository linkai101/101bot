	...
	hasPermission(message, ownerOverride = true) {
		...
		if(message.channel.type === 'text' && this.userPermissions) {
			...
			if(missing.length > 0) {
				...
				/*return oneLine`The \`${this.name}\` command requires you to have the following permissions:
					${missing.map(perm => permissions[perm]).join(', ')}
				`;*/
			}
		}
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly':
				...
				message.channel.send(guildOnlyMessage);
				return;

				//return message.reply(`The \`${this.name}\` command must be used in a server channel.`);
			case 'nsfw':
				const nsfwOnlyMessage = new Discord.MessageEmbed()
					.setColor(config.embedSettings.color)
					.setAuthor("NSFW channels only", 'https://images.emojiterra.com/google/android-nougat/512px/26a0.png', '')
					.setDescription("That command can only be used in a NSFW channel!")

					.setFooter(config.embedSettings.footer, config.embedSettings.footer_url);
				
				message.channel.send(nsfwOnlyMessage);
				return;
			...
			case 'clientPermissions': {
				if(data.missing.length === 1) {
					const noPermsMessage = new Discord.MessageEmbed()
						.setColor(config.embedSettings.color)
						.setAuthor('I do not have permission', 'https://images.emojiterra.com/google/android-pie/512px/1f6d1.png', '')
						.setDescription(`I need the following permissions for \`${this.name}\`:\n\`${permissions[data.missing[0]]}\``)
			...
			case 'throttling': {
				const cooldownMessage = new Discord.MessageEmbed()
					.setColor(config.embedSettings.color)
					.setAuthor('Cooldown', 'https://images.emojiterra.com/google/android-pie/512px/1f6d1.png', '')
					.setDescription(`You are on a cooldown for the \`${this.name}\` command!`)
